...
import { ReplaceUnprocessedEntitiesOptions } from '../../types';
import { generateStableHash } from '../../util';
import { checkLocationKeyConflict } from '../refreshState/checkLocationKeyConflict';
import { insertUnprocessedEntity } from '../refreshState/insertUnprocessedEntity';
import { updateUnprocessedEntity } from '../refreshState/updateUnprocessedEntity';
...
/**
 * When an entity provider emits a mutation over its connection, this function
 * is responsible for persisting those results.
 */
export async function persistProviderEmittedEntities(
  tx: Knex.Transaction,
  logger: LoggerService,
  options: ReplaceUnprocessedEntitiesOptions,
): Promise<void> {
  const { toAdd, toUpsert, toRemove } = await createDelta(tx, options);
...
/**
 * It's overwhelmingly common that full mutations are identical or nearly
 * identical between runs. This function computes the differences between the
 * current and desired states, to form a delta mutation instead which usually is
 * empty or very small.
 */
