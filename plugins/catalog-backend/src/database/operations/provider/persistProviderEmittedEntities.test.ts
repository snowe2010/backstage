...
import { persistProviderEmittedEntities } from './persistProviderEmittedEntities';
import { applyDatabaseMigrations } from '../../migrations';
import { DbRefreshStateReferencesRow, DbRefreshStateRow } from '../../tables';

jest.setTimeout(60_000);

describe('persistProviderEmittedEntities', () => {
  ...
  it.each(databases.eachSupportedId())(
    ...
    async databaseId => {
      ...
      await knex.transaction(tx =>
        persistProviderEmittedEntities(tx, logger, {
          type: 'full',
          sourceKey: 'config',
          items: [
            {
  ...
  it.each(databases.eachSupportedId())(
    ...
    async databaseId => {
      ...
      await knex.transaction(async tx => {
        await persistProviderEmittedEntities(tx, logger, {
          type: 'full',
          sourceKey: 'config',
          items: [
            {
